function renderChart() {
    ...
    d3.select("#gasD svg").remove();
    d3.select("#tooltip").remove();  // Remove any existing tooltip

    // Get the size of the container (it will change on window resize)
    const container = d3.select("#gasD");
    const containerWidth = container.node().getBoundingClientRect().width;
    const containerHeight = container.node().getBoundingClientRect().height;

    // Variables to quick Change names
    const thisFileName = "/Trumponomics/gas_prices.json?v=1";
    const xDomainLow = 1.5;
    const xDomainHigh = 5.5;
    const thisTitle = "US Average Gas Prices (regular)";
    const thisYTitle = "Cost per Gal in $";
    ...
    var oLink1 = "https://fred.stlouisfed.org/series/GASREGW";
    var oLink2 = "https://fred.stlouisfed.org/series/FYFSGDA188S";
    var oLinkName = "Gas Price Source";
    ...
    d3.json(thisFileName).then(data => {
        ...
        // Add gridlines
        const makeXGridlines = () => d3.axisBottom(x).ticks(8);
        const makeYGridlines = () => d3.axisLeft(y).ticks(8);
