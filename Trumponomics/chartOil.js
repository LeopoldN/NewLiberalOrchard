function renderChart() {
    ...
    d3.select("#oilD svg").remove();
    d3.select("#tooltip").remove();  // Remove any existing tooltip

    // Get the size of the container (it will change on window resize)
    const container = d3.select("#oilD");
    const containerWidth = container.node().getBoundingClientRect().width;
    const containerHeight = container.node().getBoundingClientRect().height;

    // Variables to quick Change names
    const thisFileName = "/Trumponomics/oiloutput.json?v=1";
    const xDomainLow = 4000;
    const xDomainHigh = 16000;
    const thisTitle = "US Average Oil Output per Day (000s of Barrels)";
    const thisYTitle = "output in thousands";
    const isStepCurve = false;
    const toolTipDesc = "Average output";
    ...
    var oLink1 = "https://www.eia.gov/dnav/pet/hist/leafhandler.ashx?n=pet&s=mcrfpus2&f=m";
    var oLink2 = "https://fred.stlouisfed.org/series/FYFSGDA188S";
    var oLinkName = "Oil Output Source";
    ...
    d3.json(thisFileName).then(data => {
        ...
        // Add gridlines
        const makeXGridlines = () => d3.axisBottom(x).ticks(0);
    ...
    if(isStepCurve && isTwoVariables){
        ...
        const eventDates = [
            {date: "2001-01-01", label: "Bush 1st Term"},
            {date: "2005-01-01", label: "Bush 2nd Term"},
            {date: "2009-01-01", label: "Obama 1st Term"},
            {date: "2013-01-01", label: "Obama 2nd Term"},
            {date: "2017-01-01", label: "Trump 1st Term"},
            {date: "2021-01-01", label: "Biden 1st Term"}
        ...
        const bush1Date = new Date("2000-01-01");
        const bush2Date = new Date("2005-01-01");
        const obama1Date = new Date("2009-01-01");
        const obama2Date = new Date("2013-01-01");
        ...
        .attr("width", x(obama1Date))               // Width goes to the Kennedy date
        .attr("height", height)                // Full height of the chart
        .style("fill", "red")                  // Set the color to red
        .style("opacity", backgroundOpacity);                // 50% opacity
        svg.append("rect")
        .attr("x", x(obama1Date))                          // Start from the left side
        .attr("y", 0)                          // Top of the chart
        .attr("width", x(TrumpDate)-x(obama1Date))               // Width goes to the Kennedy date
        .attr("height", height)                // Full height of the chart
        .style("fill", "blue")                  // Set the color to red
        .style("opacity", backgroundOpacity);                // 50% opacity
        svg.append("rect")
        .attr("x", x(TrumpDate))                          // Start from the left side
        .attr("y", 0)                          // Top of the chart
        .attr("width", x(BidenDate)-x(TrumpDate))               // Width goes to the Kennedy date
        .attr("height", height)                // Full height of the chart
        .style("fill", "red")                  // Set the color to red
        .style("opacity", backgroundOpacity);                // 50% opacity
        svg.append("rect")
        .attr("x", x(BidenDate))                          // Start from the left side
        .attr("y", 0)                          // Top of the chart
        .attr("width", width-x(BidenDate))               // Width goes to the Kennedy date
        .attr("height", height)                // Full height of the chart
        .style("fill", "blue")                  // Set the color to red
        .style("opacity", backgroundOpacity);                // 50% opacity
