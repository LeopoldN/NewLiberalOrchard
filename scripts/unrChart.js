function renderChart() {
    ...
    d3.select("#unrchart-container svg").remove();
    d3.select("#tooltip").remove();  // Remove any existing tooltip

    // Get the size of the container (it will change on window resize)
    const container = d3.select("#unrchart-container");
    const containerWidth = container.node().getBoundingClientRect().width;
    const containerHeight = container.node().getBoundingClientRect().height;

    // Variables to quick Change names
    const thisFileName = "/CSV/UNRATE.json?v=1";
    const xDomainLow = 2;
    const xDomainHigh = 15;
    const thisTitle = "US U-3 Unemployment Rate";
    const thisYTitle = "Percentage of Unemployment";
    const isStepCurve = false;
    const toolTipDesc = "URate";
    ...
    var oLink1 = "https://fred.stlouisfed.org/series/UNRATE";
    var oLink2 = "https://fred.stlouisfed.org/series/FYFSGDA188S";
    var oLinkName = "UNRate Source";
