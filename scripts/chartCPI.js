function renderChart() {
    ...
    svg.append("text")
        ...
        .text("US CPI Level Chained 1982")
    ...
    // Load the JSON data
    d3.json("/CSV/CPI.json?v=1").then(data => {
        data.forEach(d => {
            d.date = parseTime(d.date);
            d.cpi = d.cpi !== null ? +d.cpi : null;
        });
        ...
        const latestData = data.filter(d => d.cpi !== null).slice(-1)[0];
