function renderChart() {
    ...
    d3.select("#ManInv-container svg").remove();
    d3.select("#tooltip").remove();  // Remove any existing tooltip

    // Get the size of the container (it will change on window resize)
    const container = d3.select("#ManInv-container");
    const containerWidth = container.node().getBoundingClientRect().width;
    const containerHeight = container.node().getBoundingClientRect().height;

    // Variables to quick Change names
    const thisFileName = "/CSV/Man_invest.json?v=1";
    const xDomainLow = 45;
    const xDomainHigh = 150;
    const thisTitle = "Real Private Manufacturing Investment (in Billions)";
    const thisYTitle = "Investments (in Billions)";
    const isStepCurve = false;
    const toolTipDesc = "Investments";
    const isDollar = true;
    const isPercentage = false;
    const jsonX = "maninvst";
    ...
    var oLink1 = "https://fred.stlouisfed.org/series/C307RX1Q020SBEA";
    var oLink2 = "";
    var oLinkName = "Manufacturing Source";
    var oLinkName2 = "";
    ...
    if(isStepCurve && isTwoVariables){
        ...
        const eventDates = [
            ...
            {date: "2021-11-15", label: "Biden Infrastructure"}
        ];
        ...
        eventDates.forEach((event,index) => {
            ...
            let shift = 0;
            if(index ==1) {shift =0
            }else if(index==2) {shift=-30
            }else if(index==3) {shift=50};
        ...
        const BidenDate = new Date(eventDates[2].date);

        // Get the x-coordinate for the Obama date
        const TrumpX = x(TrumpDate);
        const BidenX = x(BidenDate)-x(TrumpDate);
        const BidenStart = x(BidenDate);
        const BidenEnd = width-x(BidenDate);
        ...
        .style("opacity", backgroundOpacity);                // 50% opacity
        svg.append("rect")
        .attr("x", BidenStart)                          // Start from the left side
        .attr("y", 0)                          // Top of the chart
        .attr("width", BidenEnd)               // Width goes to the Kennedy date
        .attr("height", height)                // Full height of the chart
        .style("fill", "blue")                  // Set the color to red
        .style("opacity", backgroundOpacity);                // 50% opacity
