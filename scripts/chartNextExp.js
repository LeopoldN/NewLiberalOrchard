function renderChart() {
    ...
    d3.select("#netexp-container svg").remove();
    d3.select("#tooltip").remove();  // Remove any existing tooltip

    // Get the size of the container (it will change on window resize)
    const container = d3.select("#netexp-container");
    const containerWidth = container.node().getBoundingClientRect().width;
    const containerHeight = container.node().getBoundingClientRect().height;

    // Variables to quick Change names
    const thisFileName = "/CSV/Net-Export.json?v=1";
    const xDomainLow = -1000;
    const xDomainHigh = -200;
    const thisTitle = "Yearly Net Exports";
    const thisYTitle = "Net Export $";
    const isStepCurve = false;
    const toolTipDesc = "Net Export";
    const isDollar = true;
    ...
    var oLink1 = "https://fred.stlouisfed.org/series/NETEXC";
    var oLink2 = "";
    var oLinkName = "Next Export Source";
    var oLinkName2 = "";
